/// See: https://github.com/dart-lang/sdk/tree/main/pkg/analysis_server/lib/src/utilities/flutter.dart
...
class Flutter {
  ...
  void convertChildToChildren2(
    ...
    if (!childArgSrc.contains(eol)) {
      ...
      var separator = getText(namedExp.offset, childArg.offset - namedExp.offset);
      ...
      var newChildArgSrc = childArgSrc.replaceAll(RegExp('^$indentOld', multiLine: true), indentNew);
  ...
  NamedExpression? findNamedExpression(AstNode node, String name) {
    ...
    if (invocation is! InstanceCreationExpression || !isWidgetCreation(invocation)) {
      return null;
    }
    return grandParent;
  }
  ...
  InstanceCreationExpression? identifyNewExpression(AstNode? node) {
    ...
    if (node is SimpleIdentifier) {
      ...
      if (parent is ConstructorName && grandParent is InstanceCreationExpression) {
        newExpr = grandParent;
      } else if (grandParent is ConstructorName && greatGrandParent is InstanceCreationExpression) {
  ...
  bool isChildArgument(Expression argument) => argument is NamedExpression && argument.name.label.name == 'child';

  /// Return `true` is the given [argument] is the `child` argument.
  bool isChildrenArgument(Expression argument) => argument is NamedExpression && argument.name.label.name == 'children';
  ...
  bool isColor(DartType type) {
    ...
    bool isColorElement(ClassElement element) {
      bool isExactColor(ClassElement element) => element.name == 'Color' && element.library.name == 'dart.ui';
  ...
  bool isDiagnosticable(DartType? type) {
    ...
    bool isDiagnosticableElement(ClassElement element) {
      bool isExactDiagnosticable(ClassElement element) =>
          element.name == 'Diagnosticable' && element.source.uri == _uriDiagnostics;
  ...
  bool isExactEdgeInsetsGeometryType(DartType type) {
    return type is InterfaceType && _isExactWidget(type.element, 'EdgeInsetsGeometry', _uriEdgeInsets);
  }
  ...
  bool isExactlyStatelessWidgetType(DartType type) {
    return type is InterfaceType && _isExactWidget(type.element, _nameStatelessWidget, _uriFramework);
  }
  ...
  bool isExactWidgetTypeBuilder(DartType type) {
    return type is InterfaceType && _isExactWidget(type.element, _nameBuilder, _uriBasic);
  }

  /// Return `true` if the given [type] is the Flutter class `Center`.
  bool isExactWidgetTypeCenter(DartType type) {
    return type is InterfaceType && _isExactWidget(type.element, _nameCenter, _uriBasic);
  }

  /// Return `true` if the given [type] is the Flutter class `Container`.
  bool isExactWidgetTypeContainer(DartType? type) {
    return type is InterfaceType && _isExactWidget(type.element, _nameContainer, _uriContainer);
  }

  /// Return `true` if the given [type] is the Flutter class `Padding`.
  bool isExactWidgetTypePadding(DartType? type) {
    return type is InterfaceType && _isExactWidget(type.element, _namePadding, _uriBasic);
  }

  /// Return `true` if the given [type] is the Flutter class `SizedBox`.
  bool isExactWidgetTypeSizedBox(DartType type) {
    return type is InterfaceType && _isExactWidget(type.element, _nameSizedBox, _uriBasic);
  }
  ...
  bool isListOfWidgetsType(DartType type) {
    return type is InterfaceType && type.isDartCoreList && isWidgetType(type.typeArguments[0]);
  }
